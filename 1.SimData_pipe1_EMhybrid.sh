...
#echo -e "python3 EMHybrid.py --INPUT_TSV "${INPUT_TSV}" --NPVAF_DIR "${NPVAF_DIR}" --CLEMENT_DIR "${CLEMENT_DIR}" --SCICLONE_DIR "${SCICLONE_DIR}" --PYCLONEVI_DIR "${PYCLONEVI_DIR}" --COMBINED_OUTPUT_DIR "${COMBINED_OUTPUT_DIR}" --CLEMENT_DIR "${CLEMENT_DIR}" --NUM_CLONE_TRIAL_START "${NUM_CLONE_TRIAL_START}" --NUM_CLONE_TRIAL_END "${NUM_CLONE_TRIAL_END}" --DEPTH_CUTOFF "${DEPTH_CUTOFF}" --VERBOSE "${VERBOSE}" --TRIAL_NO "${TRIAL_NO}" --RANDOM_SEED "${RANDOM_SEED}"  --SCORING "${SCORING}"--MODE Hard"

python3 EMhybrid.py    --INPUT_TSV ${INPUT_TSV}    --NPVAF_DIR ${NPVAF_DIR} --CLEMENT_DIR ${CLEMENT_DIR} --SCICLONE_DIR ${SCICLONE_DIR}    --PYCLONEVI_DIR ${PYCLONEVI_DIR} --QUANTUMCLONE_DIR ${QUANTUMCLONE_DIR}    --COMBINED_OUTPUT_DIR ${COMBINED_OUTPUT_DIR}      --NUM_CLONE_TRIAL_START ${NUM_CLONE_TRIAL_START}     --NUM_CLONE_TRIAL_END ${NUM_CLONE_TRIAL_END}  --FP_RATIO ${FP_RATIO}    --MAXIMUM_NUM_PARENT ${MAXIMUM_NUM_PARENT} --DEPTH_CUTOFF ${DEPTH_CUTOFF}     --VERBOSE ${VERBOSE}     --TRIAL_NO ${TRIAL_NO}   --RANDOM_SEED ${RANDOM_SEED}     --SCORING ${SCORING}   --MODE ${MODE}
