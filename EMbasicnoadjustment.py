...
def Mstep(membership, df, NUM_CLONE, NUM_BLOCK):  ## 새로운 MIXTURE 정하는 과정
...
def Outlier_detection (max_prob_list, j):
    ...
    #후보1.  Elbow 법
    threshold_x_elbow = len(max_prob_list)
    threshold_x_gaussian= len(max_prob_list)
...
def main(df, method, **kwargs):
    ...
    for NUM_CLONE in range (NUM_CLONE_TRIAL_START ,NUM_CLONE_TRIAL_END + 1 ):
        ...
        for trial in range(TRIAL_NO):
            mixture = set_initial_parameter(NUM_CLONE, trial, mixture)
            max_mixture = mixture
            ...
            for t in range(1,30):
                ...
                mixture = Mstep(membership , df, NUM_CLONE, NUM_BLOCK)        # 새 memberhsip에서 새 mixture구하기
    ...
    if method.split("+")[1] == "outlier":
        ...
        # 새로 mixture를 정해준다
        maxmaxmax_mixture = Mstep(maxmaxmax_membership , df, maxmaxmax_NUM_CLONE, NUM_BLOCK) 
