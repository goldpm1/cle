...
def makedf ():
    global NUM_BLOCK_INPUT, NUM_BLOCK, RANDOM_PICK, AXIS_RATIO, PARENT_RATIO, FP_RATIO, INPUT_DIR, INPUT_TSV, mixture_answer, membership_answer, samplename_dict, DEPTH_CUTOFF
    ...
    #df_counts = df_counts.loc[1:, list(samplename_dict.keys())]
    ...
    if NUM_PARENT != 0:
        ...
        parent_type, count = parent_type[count_sort_ind] , count[count_sort_ind] 

        parent_type_selected =  parent_type[ 0:NUM_PARENT ]

        # samplename_dict_revised, cnt = {}, 0
        # for key, value in samplename_dict.items():
        #     if ("," not in key) | (key in parent_type_selected):
        #         samplename_dict_revised[key] = cnt
        #         cnt = cnt + 1
        # samplename_dict = samplename_dict_revised
...
def random_pick_fun(**kwargs):
    ...
    if (kwargs["FP_2D"] == "True"):        # kwargs["FP"] == True면 full로 뽑는다
        FP_randomsample = FP_index
        FP_RATIO = len(FP_index) / RANDOM_PICK
    elif (kwargs["FP_2D"] == "False") & (kwargs["FP_RATIO"] == 0):      # 둘다 False, 0이면 하나도 안 뽑느다
        FP_randomsample = FP_index = []
        FP_RATIO = 0
    elif (kwargs["FP_2D"] == "False") & (kwargs["FP_RATIO"] != 0):
        try:
    ...
    try:
        ...
        print ("FP RATIO : {}\tPARENT RATIO : {}\t")
    except:
        print (INPUT_TSV + " - Can't extract Child_space data as requested")
        return False
    

    print ("조정 후  FP 개수 : {}\tPARENT 개수 : {}\tCHILD_SPACE 개수 : {}\tCHILD_AXIS 개수 : {}".format(len(FP_randomsample), len (PARENT_randomsample), len (CHILD_SPACE_randomsample), len (CHILD_AXIS_randomsample)))
    ...
    t.to_csv ("{0}.npvaf".format( kwargs["NPVAF_DIR"] ), index = True, header=True, sep = "\t")
...
def main (**kwargs):
    global NUM_BLOCK_INPUT, NUM_BLOCK, RANDOM_PICK, AXIS_RATIO, PARENT_RATIO, FP_RATIO, INPUT_TSV, SCICLONE_DIR, PYCLONE_DIR, PYCLONEVI_DIR, mixture_answer, membership_answer, DEPTH_CUTOFF
    global  df, inputdf, input_containpos, np_vaf, membership, mutation_id, depth_list, NUM_MUTATION, NUM_CLONE, NUM_BLOCK, samplename_dict, NUM_PARENT
    global membership_answer, mutation_id, mixture_answer
