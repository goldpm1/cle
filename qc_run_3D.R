...
data_df_3 <- read.table(INPUT_Third, header = TRUE)

data_df_1$Genotype <- as.factor(data_df_1$Genotype)
data_df_2$Genotype <- as.factor(data_df_2$Genotype)
data_df_3$Genotype <- as.factor(data_df_3$Genotype)

list_df_data <- mget(ls(pattern = "data_df"))

Clustering_output <- QuantumClone(list_df_data, FREEC_list = NULL, contamination = c(0,0,0), nclone_range = 2:10,
...
write.table(third, paste0(OUTPUT_DIR, "/output2.txt"), append = FALSE, sep = "\t", dec = ".", row.names = FALSE, col.names = TRUE, quote = FALSE)