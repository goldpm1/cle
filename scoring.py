...
def Scoring(membership_answer_character, membership_answer_numerical, my_membership, fp_index, parent_index, **kwargs):
    ...
    # ANSWER_NUM_CLONE이 더 많으면  이렇게 하면 안된다
    if np.max(my_membership)  >= np.max(membership_answer_numerical):      # 이러면 answer membership을 뒤섞어보면 된다
        max_index_dict = {}
        ...
        for tt in list(itertools.permutations( list(range(0, np.max(membership_answer_numerical) + 1)),  int ( np.max(my_membership) + 1) ) ):
            ...
            temp = []
            for i in range(len(my_membership)):
                temp.append(index_dict[my_membership[i]])
            ...
            # print (index_dict,  np.sum(np.equal(temp, membership_answer_numerical)))
...
def find_mindistance(j, mixture_my, mixture_target):
    ...
    for k in range(mixture_target.shape[1]):
        #print ("k = {}\tmixture_my = {}\tmixture_target = {}".format (k, mixture_my [:,j], mixture_target[:,k]))
        distance_temp.append(scipy.spatial.distance.euclidean(
            mixture_my[:, j], mixture_target[:, k]))
...
def mixturebased(mixture_left, mixture_right, membership_left, membership_right, samplename_dict_input, samplename_dict_input_rev, includeoutlier, fp_index, tool, **kwargs):
    ...
    membership_left, membership_right = list(
        membership_left), list(membership_right)
    intersect_list = []
