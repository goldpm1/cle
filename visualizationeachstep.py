    ...
    for k in sorted(list(set(bunch.membership))):     
        ...
        try:   
            ...
            if k == bunch.fp_index:  # FP를 따로 그려줌
                ax.plot(x, y, color=Gr_10[10], linewidth=5, label="FP (n = {})".format(np.bincount(bunch.membership)[k]))
                ax.text(0, np.max(y) * 1.1, "{} (n={})".format( round(mean_x, 2),  np.bincount(bunch.membership)[k]), ha="center", fontdict = {"fontsize": 16, "fontweight" : "bold"})
...
def drawfigure_1d_soft(bunch, np_vaf, output_filename, **kwargs):
    ...
    fig, ax = matplotlib.pyplot.subplots(ncols=1, nrows = 1, figsize=(6, 6))
    ax.set_xlim(0, 1)
    ax.set_xlabel ("VAF x 2 (= Mixture)")
    ax.set_ylabel ("count (weighted)")
    ...
    ax.set_title("sum_child = {}".format(list(np.round(np.sum(bunch.mixture[:, bunch.makeone_index], axis=1), 2)), bunch.makeone_index ), fontsize=12)

    for k in sorted(list(set(bunch.membership))):
        if k == bunch.fp_index:  # FP를 따로 그려줌
            sns.distplot(pd.DataFrame(np_vaf[:, 0] * 2, columns=["vaf"])["vaf"], hist_kws={"weights": membership_p_normalize[:, k], "rwidth": 0.8}, color=Gr_10[10],
                         kde=False, bins=50, label="cluster FP  (mixture = {})".format(  str(round((mixture[0][k]), 2))), ax = ax)
            ...
            sns.distplot(pd.DataFrame(np_vaf[:, 0] * 2, columns=["vaf"])["vaf"], hist_kws={"weights": membership_p_normalize[:, k], "linewidth": 1.4, "edgecolor": "black"}, kde_kws={
                         "linewidth": 5, "color": "gray"}, color=colorlist[k], kde=False, bins=50, label="cluster {}  (mixture = {})".format(k,  str(round((mixture[0][k]), 2))), ax = ax)
        else:
            sns.distplot(pd.DataFrame(np_vaf[:, 0] * 2, columns=["vaf"])["vaf"], hist_kws={"weights": membership_p_normalize[:, k], "rwidth": 0.8}, color=colorlist[k],
                         kde=False, bins=50, label="cluster {}  (mixture = {})".format(k,  str(round((mixture[0][k]), 2))), ax = ax )
...
def drawfigure_2d_soft(bunch, np_vaf, output_filename, **kwargs):
    ...
    if (kwargs["STEP"] <= (kwargs["COMPULSORY_NORMALIZATION"] )) :
        ...
        matplotlib.pyplot.suptitle("clone{}.{}-{} (raw) : {}".format(kwargs["NUM_CLONE_ITER"], kwargs["TRIAL"], kwargs["STEP_TOTAL"], round(bunch.likelihood)), fontsize=20)    
    ax.set_title("sum_child = {}".format( list ( np.round( np.sum (bunch.mixture[:, bunch.makeone_index], axis = 1), 2) ) ), fontsize = 12)
